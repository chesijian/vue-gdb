  ...
  <ul>
    <li @click.stop.prevent='toggle'>
      <p style="overflow:hidden;position:relative"  :style="{paddingLeft:level*20+6+'px'}">
        ...
        <span style="padding:10px 0 10px 45px;display:block;width: 100%;" :class="{checked:models.id==$store.state.selectProjectNode.id}">{{models.label}}</span>
      </p>
      <div v-if="open">
...
//     },
    methods:{
      ...
      selectNode(){
        ...
        this.$bus.emit('selectNode',this.models)
