<template>
	<div class="box" style="background: #000;">
		...
		<div class="querter-box">
			<div class="search-panel" style="height:40px;padding: 5px;margin-left: 10px;">
				<input style="width:90%;height:40px" type="text" v-model="searchValue" @keyup.enter="handleClick">
				<i></i>
				<button style="background: #20a5ff;margin-left: 10px;margin-top: 7px;" @click="handleClick">搜索</button>
			</div>
  ...
  methods: {
	init() {
		...
		searchService = new qq.maps.SearchService({
			...
			complete : function(results){
				...
				var poi = pois[0];
				_this.searchAddress(poi.latLng);
				// poi.location = poi.latLng.getLng()+","+poi.latLng.getLat()
				// _this.tempObj=poi;
	...
	handleClick() {
		...
		if (this.validUtil.isNotEmpty(this.searchValue)) {
			...
			//  searchService.search('东方明珠', function (status, result) {
...
.search-panel i{
  ...
  width: 26px;
  height: 26px;
  background: url('../../assets/indexImg/search.png') no-repeat center;
}
