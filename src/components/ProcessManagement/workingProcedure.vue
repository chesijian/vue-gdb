    ...
    <div  v-if="importVisable" >
      <div class="add-part" style="height: 90%;overflow: hidden;padding: 40px 20px 120px;box-sizing:border-box;">
        <p style="position:absolute;top:10px;left:20px;">
          企业检查项
          <span style="padding-left: 99px;" @click="handlerOpenSelectPurpose">返回上一级</span>
        </p>
...
<script>
  ...
  export default {
    ...
    created(){
      ...
      getConfig(this.desFormKey, this).then(config=>{
    ...
    methods:{
      ...
      deleteCatalog(type){
        ...
        this.util.confirm(deleteStr,"提示",(res)=>{
          ...
          this.util.restDelete(url+deleteId,{formKey:this.desFormKey},(res)=>{
            this.util.unmask();
            if(res.status==200){
...
//      }
        ...
        var params = {formKey:this.desFormKey,mainFormData:JSON.stringify(this.mainData),sqlTableName:"T_PROCESS",ifSaveWorkflow:false};
    ...
    saveImportData(){//导入
      ...
      let desFormKey = this.desFormKey
