      ...
      <div class="conter_left" style="padding-bottom: 55px;box-sizing: border-box;">
        <div class="conter_left_title clear">
          ...
          <div class="title-button">
                <button @click="editCheckCatalog" v-if="sessionUtil.isAllowEdit('t_InspectSafety_SET')">编辑</button>
                <button @click="addCheckCatalog" v-if="sessionUtil.isAllowAdd('t_InspectSafety_SET')">添加</button>
                <button @click="deleteCatalog(0)" v-if="sessionUtil.isAllowDelete('t_InspectSafety_SET')">删除</button>
          </div>
        </div>
        ...
        <div class="conter_right_box">
          <div class="conter_right_title" style="overflow:hidden">
            ...
            <div class="title-button" style="padding-right:20px;">
              <button @click="addCheckItem()" v-if="sessionUtil.isAllowAdd('t_InspectSafety_SET')">添加</button>
              <button @click="deleteCatalog(1)" v-if="sessionUtil.isAllowDelete('t_InspectSafety_SET')">删除</button>
              <button @click="exportIn" v-if="sessionUtil.isAllowExport('t_InspectSafety_SET')">导入</button>
            </div>
          </div>
          <div class="aside-box" style="height:100%;" ref="tablebox">
            <el-table :data="tableData" :height="tableHeightNum" border @selection-change="handleSelectionChange">
              ...
              <el-table-column prop="" label="操作" width="80">
                <template slot-scope="scope"  v-if="sessionUtil.isAllowEdit('t_InspectSafety_SET')">
                    <span>
    ...
    methods:{
      ...
      loadCompanyItem(node,resolve){//加载企业库
        console.log('-------loadCompanyItem-------')
          ...
          this.util.mask();
        console.log('-------loadCompanyItem----111---')
      ...
      addList(node) {
        console.log('-----addList-------')
          ...
          var list = {
            // parentUid :data.sign==undefined? "root":data.sign,
            // moduleType:"inspect",
            type : data.type,
...
//          this.util.restDelete("/api_v1/construction/inspect/batchDeleteCheckItem?ids="+deleteId,null,(res)=>{
      ...
      loadCheckItemList(data){
          ...
          this.util.restGet('/api_v1/construction/inspect/getCheckItemData', params, (res)=> {
              console.log("getCheckItemData======",res);
              this.util.unmask();
      ...
      saveImportData(){//导入
        ...
        this.util.mask("正在导入");
          this.util.restPost(`/api_v1/construction/inspect/checkItem/T_INSPECT_ITEM/${parentId}`, {_list: resultList}, (res)=> {
              if (res.status == 200) {
