                ...
                <div style="max-height: 495px;overflow: auto;margin-bottom: 10px;">
                    <div v-for="(item,index) in measureList" :key="index" style="padding: 10px;text-align: left;background:#fff;margin: 10px 14px;">
                        <div style="font-size: 14px;">
                            <span style="color:#5C9AD4">【{{item.lb}}】</span>
                        </div>
                        <div style="font-size: 14px;">
                            <span style="color:#5C9AD4">{{item.inspectItemName}}</span>
                            <span style="color:#FF1A56;float:right;" @click="removeItem(item,index)">删除</span>
                        </div>
...
export default {
    ...
    methods:{
        selectCheckItem(){
            this.checkItemModels.partObj=this.partObj;
            this.$refs.selectMemberFun.initData();
        ...
        removeItem(obj,index){//删除
            this.util.confirm("确定要删除该数据吗?","提示",(res)=>{
                this.util.mask("正在删除..");
                this.util.restDelete("/api_v1/construction/inspect/"+obj.id,null,(res)=>{
                    this.util.unmask();
                    if(res.status==200){
            ...
            this.measureList.forEach((item,index) => {
                if(item.isNewFlag){
                    var obj = {
                        ...
                        inspectItemUid: item.inspectItemUid,
                        ...
                        lb: item.lb,
                        planUid: item.parentUid,
                        plandetUid: item.id,
                        // rectifyPrincipalUid: item.inspectorUid,
        ...
        loaded(mapData) {
            if (this.leafletLoaded) {
                ...
                imageUrl: this.imageUrl
                 ...
                 if(this.sessionUtil.isAllowEdit('t_measure_point')||this.sessionUtil.isAllowAdd('t_measure_point')){
        ...
        refreshPicture(data) {//刷新图纸
            ...
            this.partObj=data;
            // console.log("this.partObj======",this.partObj.name);
