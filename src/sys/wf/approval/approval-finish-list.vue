...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      loadData(){
        this.util.restGet('/api_v1/wf/tasks/approval',{pageSize:10,pageIndex:1,assignee:this.sessionUtil.getUserUid()},(res)=>{
          if(res.status==200){
            if(res.data){
      ...
      openApproval(item) {
        if (this.validUtil.isNotEmpty(item.formKey)) {
          ...
          var query = { id: item.businessKey,formKey: item.formKey, _procInstId: item.procInstId, _procDefId: item.procDefId, _taskId: item.id , _taskType: item.category, _currentActId: item.actId, _taskStatus: status, isReadOnly: true }
          if (this.validUtil.isNotEmpty(item.formUrl) && this.validUtil.isNotEmpty(item.procInstId) && this.validUtil.isNotEmpty(item.id )) {
