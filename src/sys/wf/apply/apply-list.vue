<template>
    <div>
        ...
        :row-style="{overflow:'hidden',}" @current-change='openApply'
...
export default {
    ...
    methods:{
      ...
      //页面跳转
        ...
        loadData(){
            this.util.restGet('/api_v1/wf/tasks/apply',{pageSize:10,pageIndex:1,assignee:this.sessionUtil.getUserUid()},(res)=>{
                if(res.status==200){
                  if(res.data){
         ...
         openApply(item) {
          // console.info(item.formKey+'-----openApply---'+item.procInstId+'---'+item.formUrl+'---'+item.businessKey)
            if (this.validUtil.isNotEmpty(item.formKey)) {
                if (this.validUtil.isNotEmpty(item.procInstId)) {
                    if (this.validUtil.isNotEmpty(item.formUrl)) {
                        this.$router.push({path: item.formUrl,query: { id: item.businessKey, _procInstId: item.procInstId, _procDefId: item.procDefId,  isReadOnly: true } });
                    }else {
                      this.$router.push({
                        name: "page/formDataEdit",
                        query:{ id: item.businessKey, _procInstId: item.procInstId, _procDefId: item.procDefId,  isReadOnly: true },
                        params: { id: item.businessKey,
