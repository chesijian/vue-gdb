...
export const mixin = {
  ...
  created () {
    this.$bus.$on('handlerApplyReload', (params) => { //获取传递的参数并进行操作
      console.log('-------handlerApplyReload------')
      this.loadData()
    })
  },
  mounted () {
  ...
  methods: {
    ...
    openPage(column, row, index){
      ...
      if (this.validUtil.isNotEmpty(row.formKey)) {
        if (this.validUtil.isNotEmpty(row.procInstId)) {
          let query = { id: row.businessKey,formKey: row.formKey, readOnly:true, _procInstId: row.procInstId, _procDefId: row.procDefId,  isReadOnly: true }
