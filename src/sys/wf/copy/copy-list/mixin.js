...
export const mixin = {
  ...
  created () {
    this.$bus.$on('handlerCopyReload', (params) => { //获取传递的参数并进行操作
      console.log('-------handlerCopyReload------')
      this.loadData()
    })
  },
  mounted () {
  ...
  methods: {
    ...
    openPage(column, row, index){
      ...
      if (this.validUtil.isNotEmpty(row.formKey)) {
        if(this.validUtil.isNotEmpty(row.procInstId) && this.validUtil.isNotEmpty(row.id )){
          let query = { id: row.businessKey,formKey:row.formKey,readOnly:true, _procInstId: row.procInstId, _procDefId: row.procDefId, _taskId: row.id , _taskType: row.category, _currentActId: row.actId, isReadOnly: true }
          if (this.validUtil.isNotEmpty(row.formUrl)) {
