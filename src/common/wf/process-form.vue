  ...
  <div class="jrsoft-process" style="margin-top:10px;">
    ...
    <div class="buttonGroup">
      <div>
        <div style="text-align: left;" class="btnClass">
          <el-button size="mini" v-if="isApproval" @click="submit(6)" type="success">同意</el-button>
          <el-button size="mini" v-if="ifBackFirst || isCopy" @click="submit(6)" type="success">提交</el-button>
          <el-button size="mini" v-if="isWorkflow && !isApply && !isApproval   && !isCopy && !ifBackFirst" @click="submit(8)"
                     type="primary">{{isEndEvent?"办理完成":"提交"}}
          </el-button>
          <el-button size="mini" v-if="!isReadOnly && !isApproval" @click="submit(1)" type="primary">保存</el-button>
          ...
          <el-button size="mini" v-if="isApply" @click="submit(4)" type="primary">催办</el-button>
          <el-button size="mini" @click="submit(0)">返回</el-button>
        </div>
      </div>
    </div>
    ...
    <div v-if="isWorkflow">
      <div v-if="activitiData.length>0"
           style="background-color: rgb(238, 238, 238); height: 35px;line-height: 35px;padding-top: 0px;padding-left: 10px;">
              ...
              <li style="margin-top: 2px;">
                <ul style="overflow:hidden;">
                  <li
                    ...
                    <div>
                      ...
                      <div style="text-align:center; font-size: 10px;">
                        <span>{{procUser.userName.length>4?procUser.userName.substring(procUser.userName.length-4,procUser.userName.length):procUser.userName}}</span>
                      </div>
                    </div>
                  ...
                  <li v-if="ifSelUser"
                    ...
                    <div>
                      <svg class="icon" style="width:46px;height:46px;fill:#ccc;font-size:46px;color:#ccc;" @click="addProcUser()" aria-hidden="true">
                  ...
                  <li v-if="ifSelUser"
                    ...
                    <div>
                      <svg class="icon" style="width:46px;height:46px;fill:#ccc;font-size:46px;color:#ccc;" @click="delProcUser()" aria-hidden="true">
            ...
            <li style="margin-top: 2px;">
              <ul>
                ...
                <li v-if="ifSelCopy" style="float:left;margin:5px 0px 17px 3px;position:relative;text-align:left;">
                  <svg class="icon" style="width:46px;height:46px;fill:#ccc;font-size:46px;color:#ccc" @click="addCopyUser()" aria-hidden="true">
                    <use xlink:href="#icon-tianjiarenyuan"></use>
                  </svg>
                ...
                <li v-if="ifSelCopy" style="float:left;margin:5px 5px 17px 5px;position:relative;text-align:left;">
                  <svg class="icon" style="width:46px;height:46px;fill:#ccc" @click="delCopyUser()" aria-hidden="true">
                    <use xlink:href="#icon-shanchulianxiren"></use>
                  </svg>
      ...
      <el-row v-if="activitiData.length>0 || isCopy" style="margin-bottom: 0px;">
        ...
        <div style="margin-top: 5px;margin-bottom:5px;">
          <el-input type="textarea" v-model="comment" style="margin-left:0px;"></el-input>
        </div>
      </el-row>
    </div>
    ...
    <div v-if="showProcessMonitor" style="">
      ...
      <el-row>
        <div class="middle" style="background-color: rgb(238, 238, 238);line-height: 35px;">
          <div class="pop" style="background-color: #eeefef;margin-left:10px;">
            <span style="text-align:left;">流转记录</span><span class="span-look" @click="openPng();" style="">查看流程图</span>
          </div>
        </div>
        <process-monitor :model="processMonitorModel" ref="processMonitor" style="margin-top:10px;"></process-monitor>
      </el-row>
    </div>
...
<script>
  ...
  import ProcessMonitor from "@/common/wf/process-monitor.vue";
  ...
  export default {
    ...
    methods: {
      initProcess() {
        ...
        if (this.defaultConfig.ifHasProcess) {
          ...
          if (this.validUtil.isNotEmpty(this.taskId)) {
            ...
            //说明是第一次提交
            this.util.restGet("/api_v1/wf/process", {formKey: this.model.formKey, businessKey: this.id, currentUserUid: this.sessionUtil.getUserUid()}, function (res) {
                // console.info("-------说明是第一次提交" ,res);
                // 根据配置的启动规则计算流程
      ...
      addProcUser() {
        ...
        // this.$refs.selectProcUser.snyc(this.procUserModel.selectList);
      },
      removeProcUser(user) {
      ...
      submit(type) {
        ...
        if (type == 0) {
          ...
          console.debug(this.modelList.length+"==procDefId======"+this.procDefId);
        ...
        if (type == 6 || type == 8) {
          ...
          //催办
          if (this.isWorkflow) {
            console.info('-----------------'+this.model.parent.sqlTableName)
            params["sqlTableName"] = this.validUtil.isNotEmpty(this.model.parent.tableName) ? this.model.parent.tableName : this.model.parent.sqlTableName;
