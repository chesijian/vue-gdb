export const LeafletMap = function() {
    ...
    this.iconUrlArr = ['../../static/js/leaflet/lib/images/marker-icon.png','../../static/js/leaflet/lib/images/marker-icon-1.png',
    '../../static/js/leaflet/lib/images/marker-icon-2.png','../../static/js/leaflet/lib/images/marker-icon-3.png'];
    // 使用用户自己的图标
    ...
    this.initMap = function(config) {
        ...
        this.map.on(L.Draw.Event.TOOLBARCLOSED, function(event) {
            ...
            //触发事件
            // if (lmap.listeners != null && lmap.listeners[lmap.events.TOOLBARCLOSED] != undefined) {
            //     //console.info("==============");
            //     lmap.listeners[lmap.events.TOOLBARCLOSED](layer);
            // }
        });
        ...
        this.map.on(L.Draw.Event.DELETED, function(event) {
            ...
            //触发添加事件
            if (lmap.listeners != null && lmap.listeners[lmap.events.LAYER_DELETE] != undefined) {
                //console.info("==============");
                lmap.listeners[lmap.events.LAYER_DELETE](layers);
            }
        });
    ...
    this.addLayer = function(layer) {
            ...
            if (layer.entity == undefined || layer.entity == null) {
                ...
                layer.entity = entity;
                layer.entity.title=this.getUUID();
                layer.entity.uuid="point_"+Math.random().toString(36).substr(2);
            } else {
                //初始化
                entity = layer.entity;
            }
